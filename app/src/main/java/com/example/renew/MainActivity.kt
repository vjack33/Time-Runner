...
import com.example.renew.AppConfig.LATE_TIME_RESTRICTION
...
class MainActivity : AppCompatActivity() {
    ...
    var usersDBHelper = UsersDBHelper(this)                                                  // Init DB
    private val c: Calendar = Calendar.getInstance()                                                // Init Calender Object c
    val formatter: DateTimeFormatter = DateTimeFormatter.ofPattern("HH:mm")                  // Init Datetime formatter

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_main)

        // Hide/ Unhide all views
        ...
        progressBar4.visibility = INVISIBLE
        ...
        handler.post(object : Runnable {
            override fun run() {
                ...
                textViewDetailedCurrentTime.text = DateFormatSymbols().weekdays[c[Calendar.DAY_OF_WEEK]] + ", " +
                        mMonthName.substring(0, 1) +
                        mMonthName.substring(1, 3).toLowerCase() + " " +
                        mDay.toString() + " - " +
                ...
                if (sharedPreferences.getString("TODAYDATE", "") == mDate.toString()) {
                    if (sharedPreferences.getString("INTIME", "") != "") {
                        ...
                        progressBar4.visibility = VISIBLE
                        ...
                        var tvOutTime95: LocalTime =
                            tvInTime.plusMinutes(MAX_HOURS_PER_DAY.toLong())
                        ...
                        val timer = object : CountDownTimer(timeRemaining, 1000) {
                            override fun onTick(millisUntilFinished: Long) {
                                ...
                                var minutesTotalRemaining =
                                    ChronoUnit.MINUTES.between(LocalTime.now(), tvOutTime95)
                                ...
                                if (minutesTotalRemaining >= maxAvg) {
                                    ...
                                    var percentTime: Float =
                                        (((minutesTotalRemaining.toFloat() - maxAvg) / AVG_HOURS_PER_DAY) * 100)
                                    ...
                                    textViewTimeRemaining.text =
                                        ("You hv worked " + minutesExtra + "min extra. ")
                                    var percentTime: Int =
                                        ((minutesExtra.toFloat() / maxAvg) * 100).toInt()
                                    ...
                                    textViewTimeRemaining.text =
                                        ("You have exceeded 9.5 hr limit for today. ")
                                    progressBar.progress = 100
                    ...
                    if (sharedPreferences.getString("OUTDATE", "") == mDate.toString()) {
                        if (sharedPreferences.getString("OUTTIME", "") != "") {
                            textViewOutTime.text =
                                LocalTime.parse(sharedPreferences.getString("OUTTIME", ""))
                            ...
                            var finalSpentMinutes =
                                ChronoUnit.MINUTES.between(finalInTime, finalOutTime)
        ...
        imageButtonPlay.setOnClickListener {
            ...
            if (!WORKING_DAYS.contains(LocalDate.now().dayOfWeek.toString())) {
                Toast.makeText(
                    this@MainActivity,
                    "Today is not not your Working Day",
                    Toast.LENGTH_SHORT
                ).show()
            } else if (LocalTime.now() > LocalTime.parse("15:00")) {
                Toast.makeText(
                    this@MainActivity,
                    "Apply leave or Regularise its 3 PM already.",
                    Toast.LENGTH_SHORT
                ).show()
            } else {
                ...
                textViewExpt8.text =
                    LocalTime.now().plusMinutes(MIN_HOURS_PER_DAY.toLong()).format(formatter)
                        .toString()
                textViewExpt9.text =
                    LocalTime.now().plusMinutes(AVG_HOURS_PER_DAY.toLong()).format(formatter)
                        .toString()
                textViewExpt95.text =
                    LocalTime.now().plusMinutes(MAX_HOURS_PER_DAY.toLong()).format(formatter)
                        .toString()
    ...
    private fun popUpSaveToDB() {
        ...
        builder.setPositiveButton("OK",
            DialogInterface.OnClickListener { dialog, which ->
                ...
                var dataTimeSpent = ChronoUnit.MINUTES.between(
                    LocalTime.parse(dataInTime),
                    LocalTime.parse(dataOutTime)
                ).toString()
